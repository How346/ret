...
export type SendReceiptResult = { success: boolean; mode: "desktop-image" | "web-text-only"; errorType?: string };

// Sends the bill over WhatsApp.
// - Desktop (Electron): captures the receipt as an image, copies it to the
//   clipboard, and opens the WhatsApp chat — the cashier pastes (Ctrl+V) the
//   image in and sends.
// - Web build: no OS clipboard/screenshot access, so it just opens the
//   WhatsApp chat with the message text pre-filled.
export async function sendReceiptOnWhatsApp(opts: {
  ...
  if (isDesktopPrintingAvailable() && window.electronAPI?.sendReceiptWhatsApp) {
    try {
      const res = await window.electronAPI.sendReceiptWhatsApp(opts.html, opts.phone, opts.message, widthPx);
      return { success: !!res?.success, mode: "desktop-image", errorType: res?.errorType };
    } catch (err: any) {
      return { success: false, mode: "desktop-image", errorType: String(err?.message || err) };
    }
  }

  const digits = opts.phone.replace(/[^\d]/g, "");
  if (!digits) return { success: false, mode: "web-text-only", errorType: "missing-phone" };
  const url = `https://wa.me/${digits}?text=${encodeURIComponent(opts.message)}`;
