...
import { AlertTriangle, CheckCircle2, Loader2, MessageCircle, QrCode, RotateCcw } from "lucide-react";
    ...
    <Card className="p-4 space-y-3">
      ...
      {error && !ready && (
        <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-xs">
          <div className="flex items-center gap-2 font-medium text-destructive"><AlertTriangle className="h-4 w-4" /> WhatsApp connection error</div>
          <div className="mt-1 text-muted-foreground break-words">{error}</div>
          {window.electronAPI?.resetWhatsApp && (
            <Button size="sm" variant="outline" className="mt-2" disabled={starting} onClick={async () => {
              setStarting(true);
              try {
              ...
              <RotateCcw className="h-4 w-4 mr-1" /> Reset &amp; reconnect
            </Button>
          )}
        </div>
      )}
